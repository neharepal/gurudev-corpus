# Demo Questions

Curated questions that produce strong, well-grounded answers — for demos and
as informal quality anchors. Add new ones as they prove themselves.

## Known-good (produce citations across multiple works)
- What does Gurudev say about bhakti and devotion to God?
- What did Gurudev write about Kabir?
- Tell me about Carlyle Cottage — how and why was it built?
- What are the stages of sadhana in Gurudev's teaching?
- Who was Nimbargi Maharaj and what is the Inchgeri sampradaya?
- Gurudev's interpretation of the Upanishads
- आत्मज्ञानाविषयी गुरुदेव रानडे यांचे विचार काय आहेत?
- साधनेचे टप्पे कोणते? गुरुदेव काय सांगतात?

## Impromptu — 2026-07-09 small demo (audience-asked, "beautiful answers")
<!-- TODO: Neha to paste the exact impromptu questions the audience asked. -->

## Notes
- Full natural-language questions cite reliably; bare 1–2 word topics are the
  weak case (retrieval), being addressed in Phase 1A/named-work scoping.
- Reliable citations across ALL phrasings is what Plan 1B (grounding) guarantees.
