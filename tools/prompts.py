...
def build_user_message(
    ...
    if history_block:
        return (
            ...
            "bodies shown in <conversation_history>. Case (b) splits into "
            "TWO sub-modes depending on the user's ask; the OUTPUT SHAPE is "
            "different for each.\n\n"
            ...
            "INITIAL answer only. For case (b), the target passages are NOT "
            "in <retrieved_passages> — they were shown in the previous turn.\n\n"
            "(b.1) TRANSLATE / GLOSS mode — user wants the SAME passages in "
            "another language (e.g. \"translate the Marathi passages to "
            "English\", \"give me a Devanagari transliteration\"). Emit ONE "
            "citation per prior-turn passage in this shape:\n"
            ...
            "  • `quote.paraphrase` = your translation of that body in the "
            "reader's language.\n"
            "  • `quote.workTitle`, `quote.location` = copy exactly from the "
            "same prior-turn citation.\n"
            "  • `quote.kind`, `quote.author` = copy from the "
            "`[kind=..., author=...]` meta line under the citation. If "
            "missing, use `kind=\"canonical\"` + `author=\"gurudev_ranade\"`.\n"
            "  • `whyChosen` = one sentence naming the operation.\n"
            "The side-by-side card (original body + translated paraphrase) "
            "is the whole point here — do NOT collapse this to prose-only.\n\n"
            "(b.2) SUMMARIZE / EXPLAIN / ELABORATE IN YOUR OWN WORDS — user "
            "wants a NEW piece of prose about what the prior passages say "
            "(e.g. \"summarize in plain language\", \"give a plain-English "
            "summary\", \"explain what he means\", \"expand it for a lay "
            "reader\"). The OUTPUT SHAPE for this sub-mode is DIFFERENT:\n"
            "  • `framing` = ONE sentence introducing the summary. Do NOT "
            "make claims like \"Here is a summary\" without following "
            "through — if you announce content, deliver it.\n"
            "  • `framingParagraphs` = the summary itself, as an ARRAY of "
            "paragraphs (typically 3-8 paragraphs). THIS is where the body "
            "of your response goes.\n"
            "  • `synthesis` = OPTIONAL closing thought, at most 1-2 "
            "sentences. This field is a small closer. DO NOT put the main "
            "body of the summary here — that renders in a de-emphasized "
            "slot the reader may miss (observed 2026-07-19 misfire where a "
            "4,800-char summary was placed in `synthesis` and the reader "
            "asked \"where is the summary?\" for four turns).\n"
            "  • `citations` = EMPTY (`[]`). The user explicitly asked for "
            "prose, not passages. Do NOT emit b.1-style citations here — "
            "that would reproduce the verbatim source the user asked you "
            "NOT to include. If you want to reference the source works, "
            "use the `references` field with `{workTitle, location}` "
            "back-refs.\n"
            "  • If the summary announced in `framing` would not fit in "
            "any of these fields (empty framingParagraphs AND empty "
            "citations AND empty synthesis), that is an INCOMPLETE "
            "answer — do NOT emit it. Fill framingParagraphs.\n\n"
            "STICKY USER STYLE PREFERENCES (both b.1 and b.2). Look through "
            "the QUESTIONS in <conversation_history> for style directives "
            "the user gave in earlier turns, and honor them in THIS turn "
            "even if the user didn't repeat them. Examples of directives "
            "worth carrying forward:\n"
            "  • \"don't reproduce passages\" / \"don't quote verbatim\" "
            "→ stay in b.2 (prose only, empty citations) for the rest of "
            "the thread until the user explicitly changes their mind.\n"
            "  • \"in plain language\" / \"for a lay reader\" → keep the "
            "reading level accessible in this turn too.\n"
            "  • \"in ~800 words\" / \"be brief\" → carry the length "
            "target forward.\n"
            "  • \"in Marathi\" / \"answer in English\" → keep the "
            "language choice consistent.\n"
            "Sticky preferences take precedence over the sub-mode's default "
            "shape when they conflict. If the user said \"don't reproduce "
            "passages\" three turns ago and now asks \"translate that "
            "again\", that IS a conflict — the request now requires "
            "reproducing the passage. Honor the more recent, more specific "
            "ask and note the tension briefly in `framing`.\n\n"
